import { NextResponse } from "next/server";

import { getDepositSessionCookie } from "@/src/features/onboarding/lib/deposit-cookies";
import { createServerClient } from "@/src/lib/supabase/server";
import { insertNotification } from "@/src/features/notifications/lib/notifications.server";

export async function PATCH(
  request: Request,
  { params }: { params: Promise<{ id: string }> }
) {
  const { id: userId } = await params;
  const session = await getDepositSessionCookie();
  if (!session || session.user.role !== "super_admin") {
    return NextResponse.json({ error: "Forbidden: super_admin only." }, { status: 401 });
  }

  let body: { amount?: number } = {};
  try { body = await request.json() as typeof body; } catch { /* empty */ }

  if (typeof body.amount !== "number" || !isFinite(body.amount) || body.amount <= 0) {
    return NextResponse.json({ error: "A positive numeric amount is required." }, { status: 400 });
  }

  const db = createServerClient();

  const { error } = await db.rpc("admin_adjust_yield", {
    p_user_id: userId,
    p_amount:  body.amount,
  });

  if (error) {
    console.error("[yield PATCH]", error.message);
    return NextResponse.json({ error: "Yield adjustment failed." }, { status: 500 });
  }

  const fmt = new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(body.amount);
  await insertNotification({
    userId,
    type:     "system_alert",
    title:    "Yield Credit Applied",
    body:     `A manual yield credit of ${fmt} has been applied to your account.`,
    linkPath: "/wallet",
  });

  return NextResponse.json({ success: true });
}
