...
// Stitch dark tokens: glass rgba(255,255,255,0.03)+blur(12px)+border rgba(255,255,255,0.08), gold #d4a755
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden dark:bg-[rgba(255,255,255,0.03)] dark:[backdrop-filter:blur(12px)] dark:border dark:border-[rgba(255,255,255,0.08)]">
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-gray-100 flex flex-col justify-between dark:bg-[rgba(255,255,255,0.03)] dark:[backdrop-filter:blur(12px)] dark:border-[rgba(255,255,255,0.08)]">
      ...
      <div>
        ...
        <p className="text-xl font-bold text-gray-800 mt-1 dark:text-white">{value}</p>
...
const TX_ROWS = [
  {
    ...
    type: "Wallet Funding",
    typeColor: "text-gray-700 dark:text-white/80",
    desc: "Wallet activation from deposit",
    amount: "+$100.00",
    amountColor: "text-green-600 dark:text-green-400",
    status: "Completed",
    statusClass: "bg-green-50 text-green-600 dark:bg-green-500/10 dark:text-green-400",
    placeholder: false,
  },
  {
  ...
  },
  {
    ...
    typeColor: "text-gray-400 dark:text-white/30",
    desc: "-",
    amount: "$0.00",
    amountColor: "text-gray-300 dark:text-white/20",
    status: "-",
    statusClass: "",
    placeholder: true,
  },
    ...
    <div className="col-span-12 lg:col-span-8 bg-white rounded-2xl border border-gray-100 overflow-hidden dark:bg-[rgba(255,255,255,0.03)] dark:[backdrop-filter:blur(12px)] dark:border-[rgba(255,255,255,0.08)]">
      ...
      {activeTab === 0 && (
        <>
            ...
            <table className="w-full min-w-[500px] text-left">
              ...
              <tbody className="text-sm">
                {TX_ROWS.map((row, i) => (
                  <tr
                    key={i}
                    className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors dark:border-[rgba(255,255,255,0.05)] dark:hover:bg-white/5"
                  >
                    <td className="px-6 py-5 text-gray-500 font-medium text-xs dark:text-white/40">{row.date}</td>
                    ...
                    <td className="px-6 py-5 text-gray-500 text-sm dark:text-white/40">{row.desc}</td>
                    ...
                    <td className="px-6 py-5">
                      {row.status !== "-" ? (
                        ...
                        <span className="text-gray-300 text-xs dark:text-white/20">—</span>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          <div className="bg-gray-50 px-6 py-4 border-t border-gray-100 flex justify-center dark:bg-white/5 dark:border-[rgba(255,255,255,0.05)]">
            <p className="text-xs text-gray-400 font-medium dark:text-white/40">Showing 5 of 5 transactions</p>
          </div>
        </>
      )}
      ...
      {activeTab === 1 && (
        <div className="flex flex-col items-center justify-center py-20 text-center px-8">
          ...
          <p className="text-xs text-gray-400 mt-1 dark:text-white/30">Approved withdrawal requests will appear here.</p>
        </div>
      )}
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-6 dark:bg-[rgba(255,255,255,0.03)] dark:[backdrop-filter:blur(12px)] dark:border-[rgba(255,255,255,0.08)]">
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center text-sm">
          <span className="text-gray-400 dark:text-white/40">Wallet Status</span>
          <span className="font-semibold text-gray-800 dark:text-white">Active</span>
        </div>
        <div className="flex justify-between items-center text-sm">
          <span className="text-gray-400 dark:text-white/40">Activated On</span>
          <span className="font-semibold text-gray-800 dark:text-white">May 15, 2025</span>
        </div>
        <div className="flex justify-between items-center text-sm">
          <span className="text-gray-400 dark:text-white/40">Wallet ID</span>
          <span className="font-semibold text-gray-800 dark:text-white">WLT-78345</span>
        </div>
        <div className="flex justify-between items-center text-sm">
          <span className="text-gray-400 dark:text-white/40">Currency</span>
          <span className="font-semibold text-gray-800 dark:text-white">USD — US Dollar</span>
        </div>
      </div>
    </div>
  );
...
function LearnMoreContent() {
  return (
    <div className="space-y-4 text-sm text-gray-600 leading-relaxed dark:text-white/60">
      <p>
        Aurum Sovereign Capital requires a <span className="font-bold text-gray-900 dark:text-white">$100 wallet activation fee</span> from
    ...
    <div className="divide-y divide-gray-100 dark:divide-[rgba(255,255,255,0.05)]">
      {rows.map((r) => (
        <div key={r.label} className={cn("flex justify-between py-3 text-sm", r.highlight ? "font-bold" : "")}>
          ...
          <span className={cn("font-semibold", r.highlight ? "text-blue-600 dark:text-[#d4a755]" : "text-gray-800 dark:text-white")}>{r.value}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-gray-50 border border-gray-100 p-4 text-center dark:bg-white/5 dark:border-white/10">
        <p className="text-2xl font-black text-gray-800 dark:text-white">$0.00</p>
      ...
      <div className="rounded-xl border border-dashed border-gray-200 p-4 text-center dark:border-white/20">
        ...
        <p className="text-sm font-bold text-gray-800 mt-1 dark:text-white">July 1, 2026</p>
        <p className="text-[10px] text-gray-400 mt-0.5 dark:text-white/30">Cut-off: June 28 at 11:59 PM UTC</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-amber-50 border border-amber-100 p-4 text-center dark:bg-amber-500/10 dark:border-amber-500/20">
        <p className="text-2xl font-black text-amber-700 dark:text-amber-400">$0.00</p>
      ...
      <p className="text-xs text-gray-500 leading-relaxed dark:text-white/40">
        ...
        appear here with a status of <span className="font-bold text-amber-600 dark:text-amber-400">Pending Review</span> until
          ...
          >
            ...
            <div className="text-xs text-gray-500 space-y-1 dark:text-white/40">
              <div className="flex justify-between">
                <span>Cut-off</span><span className="font-semibold text-gray-700 dark:text-white/60">{s.cutoff}</span>
              </div>
              <div className="flex justify-between">
                <span>Processing</span><span className="font-semibold text-gray-700 dark:text-white/60">{s.processing}</span>
              </div>
            </div>
          </div>
        ))}
